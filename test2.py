...
lines.append(line(0 + 0j, 1 + 1j))
lines.append(line(1 + 0j, -1 + 1j))
...
def z2imgPoint(z):
    return (width / 2 + int(np.real(z * 100)), height / 2 - int(np.imag(z * 100)))
...
for key in faceKeys:
    ...
    print key, z2imgPoint(v1), z2imgPoint(v2), z2imgPoint(v3), z2imgPoint(v4)
